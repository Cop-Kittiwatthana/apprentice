...
$query2 = "SELECT teacher.t_id,teacher.t_fna,teacher.t_lna ,branch.br_na 
    ...
    WHERE  student.s_group = '$s_group'  and student.br_id= '$br_id' and student.s_id LIKE '$s_id%'
...
if (mysqli_num_rows($result1) > 0) {
    ...
    while ($row2 = mysqli_fetch_assoc($result2)) {
        ...
        $tableh .= 'ครูที่ปรึกษา' . ' ' . $t_tna . '' . $row2['t_fna'] . ' ' . $row2['t_lna'].'<br>';
    }'<h5>';
    $tableh .= '<br>';
    $tableh .= '
<table id="bg-table" width="100%" style="border-collapse: collapse;font-size:16pt;">
  ...
  <tbody>';
    ...
    echo "<script> alert('กรุณาตรวจสอบข้อมูลอาจารย์ที่ปรึกษาก่อน'); window.close()</script>";
    exit();
}
