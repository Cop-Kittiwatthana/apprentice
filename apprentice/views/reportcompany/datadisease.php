...
$query .= " WHERE company.c_id = '$c_id' and";
...
while ($row = mysqli_fetch_array($result)) {
	...
	$nestedData = array();
	if ($row["pe_semester"] != "") {
		$pe_semester = (int)$row["pe_semester"];
		$nestedData[] = ($pe_semester).'-'.($pe_semester+1) ;
	} else {
		$nestedData[] = '<font color="red" style="text-align:center;">ยังไม่มีข้อมูล</div></font>';
	}
	//$nestedData[] = $row["total"];
	$nestedData[] = $row['detail'] = '<div align="center">' . "<a href=\"indexdiseasepdf.php?c_id=$row[c_id]&pe_semester=$date&su_term=1\">" . '<button type="button" class="btn btn-info">&nbsp;&nbsp;&nbsp;<i class="fas fa-info-circle"> เทอม1&nbsp;&nbsp;&nbsp;</i></button></a>
	' . "<a  href=\"indexdiseasepdf.php?c_id=$row[c_id]&pe_semester=$date&su_term=2\">" . '<button type="button" class="btn btn-info">&nbsp;&nbsp;&nbsp;<i class="fas fa-info-circle"> เทอม2&nbsp;&nbsp;&nbsp;</i></button>';
	$data[] = $nestedData;
}
