...
// *****************************************************************************************************************************
$query1 = "SELECT student.s_id,student.s_fna,student.s_lna,student.s_tel,branch.br_na
FROM petition  
-- LEFT JOIN supervision ON petition.pe_id  = supervision.pe_id 
LEFT JOIN demand ON petition.de_id  = demand.de_id  
LEFT JOIN company ON demand.c_id  = company.c_id 
...
if (mysqli_num_rows($result1) > 0) {
    if (mysqli_num_rows($result1) > 0) {
        ...
        while ($row1 = mysqli_fetch_assoc($result1)) {
            ...
            <td style="border-right:1px solid #000;padding:3px;">' . $row1['br_na'] . '</td>
            <td style="border-right:1px solid #000;padding:3px;">' . $row1['s_tel'] . '</td>
...
<table id="bg-table" width="100%" style="border-collapse: collapse;font-size:14pt;">
    <tr style="border:1px solid #000;padding:4px;">
        ...
        <td  width="15%" style="border-right:1px solid #000;padding:4px;text-align:center;"width="28%"><b>ชื่อสกุล</b></td>
        <td  style="border-right:1px solid #000;padding:4px;text-align:center;"  width="25%"><b>สาขา</b></td>
        <td  style="border-right:1px solid #000;padding:4px;text-align:center;" width="15%"><b>เบอร์โทร</b></td>
        <td  style="border-right:1px solid #000;padding:4px;text-align:center;" width="17%"><b>หมายเหตุ</b></td>
    </tr>
</thead>
  <tbody>';
    $tableend = "</tbody>
