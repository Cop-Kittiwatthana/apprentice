...
WHERE dp_id = '$dp_id'";
  ...
  while ($row = mysqli_fetch_array($result_department)) {  // preparing an array
    $dp_na = "$row[dp_na]";
  }
...
WHERE br_id = '$br_id'";
  $result_branch = mysqli_query($conn, $query_branch)
    or die("3. ไม่สามารถประมวลผลคำสั่งได้") . $mysql->error();
  $data = array();
  while ($row = mysqli_fetch_array($result_branch)) {  // preparing an array
    $br_na = "$row[br_na]";
  }
?>
  <!DOCTYPE html>
          ...
          <form action="sql.php" method="post" enctype="multipart/form-data" name="form1" id="form1">
            <div class="container text-dark " style="width: 60%; height: auto;">
              <div class="form-group">
                ...
                <input class="form-control" placeholder="-กรอกชื่อสาขา-" onkeypress="isInputChar(event)" required type="text" name="br_na" id="br_na" value="<?= $br_na ?>">
              </div>
